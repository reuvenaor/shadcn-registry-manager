...
import { updateFilesOptionsSchema } from "@/src/schemas/update.schemas"

export async function updateFiles(
  files: RegistryItem["files"],
  config: Config,
  options: z.infer<typeof updateFilesOptionsSchema>,
  ...
  const validatedOptions = updateFilesOptionsSchema.parse(options)

  const filesCreatedSpinner = spinner(`Updating files.`, extra, "update-files").start()
    ...
    const content = await transform(
      {
        ...
        isRemote: validatedOptions.isRemote,
      },
      [
        transformImport,
        transformRsc,
  ...
  if (filesUpdated.length) {
    filesCreatedSpinner?.info(`Updated ${filesUpdated.length} ${filesUpdated.length === 1 ? "file" : "files"
      }:`)
    if (!validatedOptions.silent) {
  ...
  if (filesSkipped.length) {
    filesCreatedSpinner?.info(`Skipped ${filesSkipped.length} ${filesUpdated.length === 1 ? "file" : "files"
