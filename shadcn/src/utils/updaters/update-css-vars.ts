...
import { updateCssVarsOptionsSchema } from "@/src/schemas/update.schemas"
...
export async function updateCssVars(
  ...
  config: Config,
  options: z.infer<typeof updateCssVarsOptionsSchema>,
  ...
  const validatedOptions = updateCssVarsOptionsSchema.parse(options)
  ...
  const cssVarsSpinner = spinner(
    `Updating CSS variables in ${cssFilepathRelative}`,
    extra,
    "update-css-vars"
  ).start()
  const raw = await fs.readFile(cssFilepath, "utf8")
  let output = await transformCssVars(raw, cssVars ?? {}, config, validatedOptions)
...
export async function transformCssVars(
  ...
  if (validatedOptions.cleanupDefaultNextStyles) {
    plugins.push(cleanupDefaultNextStylesPlugin())
  }

  if (validatedOptions.tailwindVersion === "v4") {
    plugins = []
    ...
    plugins.push(
      updateCssVarsPluginV4(cssVars, {
        overwriteCssVars: validatedOptions.overwriteCssVars,
      })
    )
    plugins.push(updateThemePlugin(cssVars))

    if (validatedOptions.tailwindConfig) {
      plugins.push(updateTailwindConfigPlugin(validatedOptions.tailwindConfig))
      plugins.push(updateTailwindConfigAnimationPlugin(validatedOptions.tailwindConfig))
      plugins.push(updateTailwindConfigKeyframesPlugin(validatedOptions.tailwindConfig))
    }
  }

  if (config.tailwind.cssVariables && validatedOptions.initIndex) {
    plugins.push(
      updateBaseLayerPlugin({ tailwindVersion: validatedOptions.tailwindVersion })
    )
  }
